<template>
  <div class="ser_home">
    <ul class="red_packet" id="red_packet">
      <li
          v-for="(item, index) in liParams" :key="index"
        ...
        >
          <a href="javascript:;">
            <i :style="{ transform: item.transforms, webkitTransform: item.transforms}" class="iicon"></i>
          </a>
        </li>
    </ul>
  </div>
...
export default {
  ...
  methods: {
    ...
    startRedPacket () {
      ...
      // 旋转角度
      let scales = (Math.random() * (12 - 8 + 1) + 8) * 0.1
      // 图片尺寸
      let durTime = Math.random() * (2.5 - 1.2 + 1) + 1.2 + "s"
      // 时间  1.2和1.2这个数值保持一样
      console.log(durTime)
      this.liParams.push({
        left: left + "px",
